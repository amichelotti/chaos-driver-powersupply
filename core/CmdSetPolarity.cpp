...
BATCH_COMMAND_OPEN_DESCRIPTION_ALIAS(driver::powersupply::, CmdSetPolarity, CMD_PS_SET_POLARITY_ALIAS, "Set the polarity ", "adf2973c-35dd-11e5-855a-734defc0b6db")
...
void own::CmdSetPolarity::setHandler(c_data::CDataWrapper *data) {
  ...
  if (!(feature & ::common::powersupply::POWER_SUPPLY_FEAT_MONOPOLAR)) {
    ...
    metadataLogging(chaos::common::metadata_logging::StandardLoggingChannel::LogLevelWarning, "this command is invalid for bipolar types");
  ...
  if ((err = powersupply_drv->setPolarity(polarity_set_point)) != 0) {
    ...
    setStateVariableSeverity(StateVariableTypeAlarmCU, "polarity_invalid_set", chaos::common::alarm::MultiSeverityAlarmLevelHigh);
...
bool own::CmdSetPolarity::timeoutHandler() {
  ...
  if (polarity_set_point == *o_pol) {
    ...
    setStateVariableSeverity(StateVariableTypeAlarmCU, "polarity_value_not_reached", chaos::common::alarm::MultiSeverityAlarmLevelWarning);
