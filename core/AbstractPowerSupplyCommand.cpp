...
void AbstractPowerSupplyCommand::setHandler(c_data::CDataWrapper* data) {
  CMDCUDBG_ << "setting ";
  feature= ((SCPowerSupplyControlUnit*)getControlUnit())->feature;
...
void AbstractPowerSupplyCommand::acquireHandler() {
  ...
  //CMDCUDBG_ << "current:" << *o_current << ",current_sp:" << *i_current << ",polarity:" << ((o_pol==NULL)?-1000:*o_pol) << ",alarms:" << *o_alarms << ",stby -> " << *o_stby;
